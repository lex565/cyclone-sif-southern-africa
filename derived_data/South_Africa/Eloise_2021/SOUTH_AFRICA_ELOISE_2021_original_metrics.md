# Cyclone Eloise (Jan 2021) — original manuscript metrics (Section 3)
...
|---|---|---|---|
| 409,544.8 | 1,219,470 | 0.336 | 3.0 |

Only 34% of South_Africa lies inside the 200 km storm footprint; a national mean would dilute the signal ~3x (DF).

## Eq 6 — % change vs baseline (naive vs corrected)
Naive uses the depressed *observed* baseline; corrected anchors to the climatological baseline level (0.145) on de-meaned anomalies.
...
- **Validation deferred — GPP not yet downloaded for South_Africa.** The SIF disruption analysis (Eq 1/2/5/6/7, §3.7, attribution Eq 9) is complete and fully independent of GPP. Eq 8 will be added once the PML-V2 tiles arrive, as an additive re-run with no SIF recomputation.
