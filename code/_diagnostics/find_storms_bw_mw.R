# Find SI-basin storms (2018-2021) whose track LINE intersects Botswana / Malawi.
# Reports SID, name, season, entry date (first in-country fix or closest approach),
# n in-country fixes, max wind in-country. Mirrors the Zimbabwe selection logic.
...
for (cn in c("Botswana","Malawi")) {
  ...
  for (sid in unique(pts$SID)) {
    ...
    # nearest approach (km) for context even if no fix strictly inside
    ...
    hits <- hits + 1
    cat(sprintf("  %-12s %s  SID=%s  season=%d | in-country fixes=%d | nearest=%.1f km | entry=%s | wind~%s kt\n",
                nm, "", sid, seas, sum(inside), d_km, format(entry, "%Y-%m-%d %H:%MZ"),
                ifelse(is.finite(wmax), round(wmax), "NA")))
  }
  if (hits == 0) cat("  (no SI-basin track line intersects this country in 2018-2021)\n")
}
cat("\nDONE\n")
