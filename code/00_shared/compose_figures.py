...
def compose(figname, events, mode, letters=None, per_row=1,
    ...
    fixed = hdr_top + cbar_zone + nr * (hdr_row + date_h) + (nr - 1) * gap_row
    ...
    def col_x(b, j):
        return margin_l + b * (block_w + gap_blk) + j * (col_w + gap_col)
    ...
    for g, rh in zip(groups, rows_h):
        ...
        for b, i in enumerate(g):
            ...
            fig.text(FX(col_x(b, 0)), FY(y + 0.7 * MM), hdr, ha="left",
...
def write_captions():
    ...
    txt = ["# Corrected figure captions\n",
           "Generated by `compose_figures.py`. Percent-change colour scale is "
           "shared across Figures 3–8.\n"]
